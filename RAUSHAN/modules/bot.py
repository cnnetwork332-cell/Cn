...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"•[ 🍃 𝐏ᴇᴀᴄᴇ ᴘᴀᴘᴀ σᴘ 🍃 ]•")
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"[🍹] 𝐏ᴇᴀᴄᴇ вααᴘ кє gυℓαм\n[🏓] ɪᴊᴊᴀт ѕє ʀαниα\n[⚡] 𝐖ᴀʀɴᴀ 𝐅ᴀᴛʜᴇʀ ɪꜱ 𝐑ᴇᴀᴅʏ\n\n➜ `{mp} ms`")
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"»🍃 **𝐖єℓᴄσмє gαᴅᴅαʀɪ иαнɪ кαʀиα иαнɪ тσ вαᴅмσѕнɪ мα кαʀυgα αυʀ ααᴊ ѕє тυ нαмℓσg кα внαɪ** 🍃")
        target = ""
